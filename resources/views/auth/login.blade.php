...
{{--                </div>--}}
                ...
                <div class="registration__container">
                    ...
                    <h2 class="registration__title">{{ __('Вхід') }}</h2>
                    </div>
                    <div class="registration__inner">
                        <div class="registration__aligner">
                            <div class="registration__aligner__inner">
                                <h3 class="registration__subtitle">{{ __('За допомогою пошти') }}</h3>
                                <form method="POST" action="{{ route('login') }}">
                                    @csrf
                                    <div class="row">
{{--                                        <label for="email" class="col-md-4 col-form-label text-md-right">{{ __('E-Mail Address') }}</label>--}}
...
{{--                                        <label for="password" class="col-md-4 col-form-label text-md-right">{{ __('Password') }}</label>--}}
                                    ...
                                    <div class="registration__links__container">
                                        <div class="registration__remember__container">
                                            <input class="uk-checkbox" type="checkbox" name="remember" id="remember" {{ old('remember') ? 'checked' : '' }}>

                                            <label class="registration__remember__check-label" for="remember">
                                                {{ __("Запам'ятати") }}
                                            </label>
                                        </div>

                                        @if (Route::has('password.request'))
                                            <a class="registration__links__forgot-pass" href="{{ route('password.request') }}">
                                                {{ __('Забули пароль?') }}
                                            </a>
                                        @endif
                                    </div>
